...
# set for each run
base_dir = '//oak-smb-giocomo.stanford.edu/groups/giocomo/fcho2/pilot_2023/'
prefix = 'all' #eg datetime.today().strftime('%Y%m%d')

# should be the same across all experiments
rec_file_list = os.path.join(base_dir,'Preprocessed_Data/Provenance',prefix+'_sessions_fcho_probesurvey.csv')
...
# catGT_cmd_string = '-t_miss_ok -zerofillmax=500 -out_prb_fld -gfix=0.4,0.10,0.02 -apfilter=butter,12,300,10000 '  # this was missing -prb_fld!!!
catGT_cmd_string = '-t_miss_ok -zerofillmax=500 -prb_fld -out_prb_fld -gfix=0.4,0.10,0.02 -apfilter=butter,12,300,10000 '
...
#2023: LT: [12:28] remapping: np.arange(28,108,4) #np.arange(109,347,4) #349 OR 350 #np.arange(354,428,4) #429: 
    #try:
    print(row['File'])
