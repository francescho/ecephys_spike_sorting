...
base_dir = '//oak-smb-giocomo.stanford.edu/groups/giocomo/fcho2/2024_Fall/'
prefix = 'all' #eg datetime.today().strftime('%Y%m%d')

# should be the same across all experiments
rec_file_list = os.path.join(base_dir,'Preprocessed_Data/Provenance',prefix+'sess_f24_cedartest.csv')
# rec_file_list = os.path.join(base_dir,'Preprocessed_Data/Provenance',prefix+'sess_f24_durmasttest.csv')
...
log_file = os.path.join(base_dir, 'Preprocessed_Data/Provenance',prefix+'_ecephys_log_local.csv')
...
# Dec 2023 update: new catGT requires 2 leading zeros + xa must be lowercase
# -- Yggdrasil inputs: --
...
for a, row in sessions.iterrows():
#for a, row in sessions.iloc[[0,1,3]].iterrows():
    #try:
    print(row['File'])
    ...
    ## 10/22/24: specify which probes
    # pull self.whichprobes from .csv so it can be specified per run
    # to run imec0&1: '0,1'
    # to run imec0 only: '0'
    # to run imec1 only: '1,1'
    whichprobes = str(row['whichprobes'])
    print('=== WHICHPROBES? : ' , whichprobes)


    ## 10/22/24: 
    # Set which modules to run per probe; catGT and TPrime are called once for each run
    run_CatGT = row['run_CatGT']
    run_KS = row['run_KS']
    run_TPrime = row['run_TPrime']
    module_names = ['kilosort_helper','kilosort_postprocessing','noise_templates','mean_waveforms','quality_metrics','depth_estimation','prePhy_filters']
    modules = []
    ...
    run_specs = [				
                            ...
                            [rec_file_stem, '0,9', '0,0', whichprobes, ['cortex','cortex']]                            
        ...
        for i, prb in enumerate(prb_list):
            ...
            for shank in range(nshanks):
                if nshanks==1:
                    ...
                    chanMap = os.path.join(imro_dir, imroSelect+'.mat')
                else:
